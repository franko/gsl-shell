...
local function Asget(k1, k2, e)
   ...
   local smat = [1/a1, a1, 0, 0; 0, 0, 1/a2, a2; 1, 1, -1, -1; 0, 1, 0, 0]
...
local function As_mat_compute(roots)
   local n = #roots
   local m = matrix.calloc(n, 4)
   for i=1, n do
      local e = get_root(i)
      local k1, k2 = ks(e)
      local A1, A2, B1, B2 = Asget(k1, k2, e)
      local nc = sqrt(1 / phi_norm(e))
      m[i, 1:4] = nc * [A1, A2, B1, B2]
   end
   return m
end
...
local function feval(i, x)
   ...
   if e > v2 then
      if x < 0 then
         local k1 = sqrt(2*(e-v1))
         local A1r, A1i = As_coeff(i, 1)
         local A2r, A2i = As_coeff(i, 2)
         local c, s = cos(k1*x), sin(k1*x)
         return A1r*c + A1i*s + A2r*c - A2i*s, -A1r*s + A1i*c + A2r*s + A2i*c
      else
         local k2 = sqrt(2*(e-v2))
         local B1r, B1i = As_coeff(i, 3)
         local B2r, B2i = As_coeff(i, 4)
