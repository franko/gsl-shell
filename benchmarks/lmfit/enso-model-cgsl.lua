...
local function fdf_generate(dataset)
   ...
   return function(x, f, J)
	     ...
	     local b7 = cgsl.gsl_matrix_get(x, 7, 0)
	     local b8 = cgsl.gsl_matrix_get(x, 8, 0)
		...
		for i = 0, n-1 do
		   ...
		   cgsl.gsl_matrix_set (f, i, 0, Yi - y)
		end
	     end
		...
		for i = 0, n-1 do
		   ...
		   cgsl.gsl_matrix_set(J, i, 3, -b4*(2*pi*t/(b3*b3))*sin(2*pi*t/b3) + b5*(2*pi*t/(b3*b3))*cos(2*pi*t/b3))
		   cgsl.gsl_matrix_set(J, i, 4, -cos(2*pi*t/b3))
		   cgsl.gsl_matrix_set(J, i, 5, -sin(2*pi*t/b3))
		   cgsl.gsl_matrix_set(J, i, 6, -b7 * (2*pi*t/(b6*b6)) * sin(2*pi*t/b6) + b8 * (2*pi*t/(b6*b6)) * cos(2*pi*t/b6))
