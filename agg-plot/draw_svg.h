...
svg_vertex_flip(VertexSource* vs, double* x, double* y, double h)
{
  unsigned cmd = vs->svg_vertex(x, y);
  *y = svg_y_coord(*y, h);
  return cmd;
}
...
void svg_coords_from_vs(VertexSource* vs, str& s, double h)
{
  ...
  while ((cmd = svg_vertex_flip(vs, &x, &y, h)))
    {
	...
	double x1 = x, y1 = y;
	double x2, y2;
	svg_vertex_flip(vs, &x2, &y2, h);
	svg_vertex_flip(vs, &x, &y, h);
