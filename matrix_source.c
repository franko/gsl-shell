...
TYPE (gsl_matrix) *
FUNCTION (push_matrix, raw) (lua_State *L, int n1, int n2)
{
  return TYPE (_push_matrix) (L, n1, n2, false);
}

TYPE (gsl_matrix) *
TYPE (push_matrix) (lua_State *L, int n1, int n2)
{
...
{
  const TYPE (gsl_matrix) *a = TYPE (check_matrix) (L, 1);
  TYPE (gsl_matrix) *copy;
  copy = FUNCTION (push_matrix, raw) (L, a->size1, a->size2);
  FUNCTION (gsl_matrix, memcpy) (copy, a);
  return 1;
}
...
{
  const TYPE (gsl_matrix) *m = TYPE (check_matrix) (L, 1);
...
{
  TYPE (gsl_matrix) *m = TYPE (check_matrix) (L, 1);
  lua_Integer r = luaL_checkinteger (L, 2);
  lua_Integer c = luaL_checkinteger (L, 3);
...
FUNCTION(matrix, new) (lua_State *L)
{
  ...
  TYPE (push_matrix) (L, (size_t) nr, (size_t) nc);
...
FUNCTION(matrix, mul) (lua_State *L)
{
  ...
  r = TYPE (check_matrix) (L, 1);

  for (k = 2; k <= nargs; k++)
    {
      a = r;
      b = TYPE (check_matrix) (L, k);

      if (a->size2 != b->size1)
	luaL_error (L, "incompatible matrix dimensions in multiplication");

      if (k == nargs)
	r = TYPE (push_matrix) (L, a->size1, b->size2);
      else
...
FUNCTION(matrix, inverse) (lua_State *L)
{
  ...
  inverse = FUNCTION (push_matrix, raw) (L, n, n);
...
{
  const TYPE (gsl_matrix) *a = TYPE (check_matrix) (L, 1);
  const TYPE (gsl_matrix) *b = TYPE (check_matrix) (L, 2);
  TYPE (gsl_matrix) *x;
  ...
  x = FUNCTION (push_matrix, raw) (L, n, 1);
